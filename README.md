...
A CLI and MCP server for the **Library of Congress JSON API**, defaulting to the
...
The dedicated `chroniclingamerica.loc.gov` API that most existing tooling targets
has been retired. Chronicling America is now reachable only as one collection of
loc.gov, so this client is written against the general API and merely *defaults*
to that collection. The same commands search the rest of loc.gov — books and
manuscripts are full-text indexed too — via `--collection` or `--all-loc`.
...
```
$ locgov search '"Brooklyn Bridge"' --from-year 1883 --to-year 1883 --per-page 2
...
`--title`, `--collection`, `--all-loc`, `--level`, `--per-page`, `--sort`.
...
that is then memoised on disk. OCR downloads are cached under
`$XDG_CACHE_HOME/loc-mcp`.
